...
async function fetchRequestById(client, id) {
  const { rows } = await client.query("SELECT * FROM requests WHERE id = $1", [id]);
  return rows[0] || null;
}

async function isCredentialAuthorizedForRequest(client, request, cpf, validationKey) {
  const cleanCpf = normalizeCpf(cpf);
  const cleanKey = String(validationKey || "").trim();
  if (cleanCpf.length !== 11 || !cleanKey) return false;

  const requestCpf = normalizeCpf(request?.cpf);
  if (requestCpf !== cleanCpf) return false;
...
function animalFromRequest(request, index = 0) {
  const animals = Array.isArray(request?.animals) ? request.animals : [];
  return animals[Math.max(Number(index) || 0, 0)] || {
    name: request?.animal_name,
    species: request?.species,
    size: request?.size,
    microchip: request?.animal_microchip,
  };
}
...
router.post("/requests/:requestId/death", async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const source = await fetchRequestById(client, req.params.requestId);
    if (!source) {
      await client.query("ROLLBACK");
      return res.status(404).json({ error: "Solicitacao original nao encontrada." });
    }

    const authorized = await isCredentialAuthorizedForRequest(client, source, req.body?.cpf, req.body?.validationKey);
    if (!authorized) {
      await client.query("ROLLBACK");
      return res.status(403).json({ error: "Solicitacao nao autorizada." });
    }
    ...
    const animal = animalFromRequest(source, req.body?.animalIndex);
    const microchip = normalizeMicrochip(pick(animal.microchip, source.animal_microchip));
    const linkedAnimal = microchip ? await ensureAnimalFromLegacyRequest(client, microchip) : null;
    const protocol = await nextProtocol(client);
    const validationKey = await validationKeyForCpf(client, source.cpf);
    ...
    const { rows } = await client.query(
      ...
      [
        ...
        source.state,
        source.cep,
        linkedAnimal?.id || source.animal_id || null,
        microchip || source.animal_microchip || null,
        pick(animal.name, source.animal_name),
        pick(animal.species, source.species),
        pick(animal.size, source.size),
        JSON.stringify([{ ...animal, microchip: microchip || animal.microchip || "" }]),
        notes,
        deathDate,
        deathCause,
...
router.post("/requests/:requestId/transfer", async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const source = await fetchRequestById(client, req.params.requestId);
    ...
    const workflowData = { animal_request_type: "transfer", source_request_id: source.id, target_tutor: target };
